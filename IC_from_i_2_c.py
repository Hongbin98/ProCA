...
def get_confi_classes(source_model, target_data_loader, threshold=0.2):
    source_model.eval()
    prediction_bank = torch.zeros(1, 1000).cuda()
    ...
    return confi_class_idx, prediction_bank[confi_class_idx]
...
if __name__ == '__main__':
    ...
    imgNet_dataset = Imagenet_Dataset(
        path='/mnt/cephfs/mixed/dataset/imagenet',
        domains=['train'],
        files=[
            '/mnt/cephfs/home/linhongbin/UDA/dataset/ImageNet-Caltech/imagenet_list.txt',
        ],
        prefix='/mnt/cephfs/mixed/dataset/imagenet',
        is_join=False
    )
    ...
    source_file = imgNet_dataset.files[0]
    target_file = cal_dataset.files[0]
    ...
    cal_84_cls_list = [713, 620, 784, 566, 593, 417, 947, 340, 9, 814, 39, 870, 779, 334, 839, 879, 852, 910, 1, 907,
                       604, 171, 760, 508, 354, 928, 872, 621, 308, 546, 113, 671, 664, 51, 71, 764, 413, 367, 414, 145,
                       933, 574, 366, 920, 594, 288, 806, 314, 934, 315, 112, 876, 543, 560, 472, 504, 79, 355, 845, 99,
                       94, 69, 435, 849, 571, 441, 479, 311, 327, 555, 579, 805, 471, 24, 148, 859, 361, 745, 515, 605,
                       637, 651, 895, 447]
    ...
    for incre_idx in range(len(cal_84_cls_list) // incre_cls_nums):  # only for the first stage
        ...
        source_total_ds = FileListDataset(list_path=source_file, path_prefix=imgNet_dataset.prefixes[source],
        ...
        target_train_ds = FileListDataset(list_path=target_file, path_prefix=cal_dataset.prefixes[target],
                                          return_id=True,
                                          transform=transform_test,
        ...
        target_test_ds = FileListDataset(list_path=target_file, path_prefix=cal_dataset.prefixes[target],
                                         transform=transform_test,
        ...
        confi_class_idx, confi_class_values = get_confi_classes(pretrained_net,
                                                                target_train_dl,
                                                                threshold=0.15)
        print(target_train_classes)
        print(confi_class_idx)
        print(confi_class_values)
        print('From {} to {}, the source-only accuracy is:'.format(imgNet_dataset.domains[source],
        ...
        # # debug
        # debug_cnt = 0
        source_centers_update_interval = True
        ...
        best_acc_stages.append(best_tar_acc / 100)
